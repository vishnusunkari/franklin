...
All validatable fields are initially set to isValid=false and validated live onchange.  Focused invalid fields show absolutely positioned error messages.
...
function setValidation(node,msg,validator,required) {
	...
	if(typeof validator != "undefined" && validator) var validationFunction = (String(validator).match(/^\/.*/)) ? function (e) { (!eval(e.target.isRequired) && e.target.value == '') ? makeValid(e.target) : e.target.value.match(validator) ? makeValid(e.target) : makeInvalid(e.target); } : function(e) { (!eval(e.target.isRequired) && e.target.value == '') ? makeValid(e.target) : validator(e) };
	//if validator is not defined or set to false, set isRequired and validate against null value
	else var validationFunction = function(e) { (!eval(e.target.isRequired) && e.target.value == '') ? makeValid(e.target) : (e.target.value == '') ? makeInvalid(e.target) : makeValid(e.target);};
...
function isNotFocused(e) {
	...
	if(typeof tempVal != "undefined" && node.value != tempVal) $(node).change();
